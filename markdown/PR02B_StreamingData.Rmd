...
waffle(round(100 * prop.table(table(disney_data$type))),title = "Streaming Content on Disney+", colors = c("darkseagreen3", "darkseagreen4"), xlab="1 square = ?") +
theme(plot.title = element_text(hjust = 0.5))
#Still need below:
#Use an x-axis label to indicate roughly how many programs each square represents I understand notation but would like to know how to decide what each square scale should be 
#how to figure out best number of rows 
```
...
```{r showsbarplot}
#streaming_data %>%
#my_table2 <- table(Hulu$Title)
#ggplot(my_table2,aes(x = frequency(Title), y = Title)) +
  #geom_col()+
 # facet_grid(.~	Netflix,Hulu,Prime Video,Disney+)
 # #labs(x = "Frequency", y = "Ratings", title = "Frequency of Movie and TV Shows by Ratings on Disney+") + 
  #theme(plot.title = element_text(hjust = 0.5))
```
